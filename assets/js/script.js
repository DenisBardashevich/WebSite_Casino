...
document.addEventListener("DOMContentLoaded", function () {
  let faqItems = document.querySelectorAll(".faqBlock__listItem");
...
document.querySelectorAll('.faqBlock__listItemTitle').forEach(function(item) {
  item.addEventListener('click', function() {
    const desc = this.nextElementSibling; // Получаем следующий элемент, который содержит ответ

    // Переключаем класс .open для блока с ответом
